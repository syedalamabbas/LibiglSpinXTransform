...
void LinearSolver :: solve(spinX::QuaternionMatrix& A,
                     vector<spinX::Quaternion>& x,
                     const vector<spinX::Quaternion>& b,
                     bool precondition )
...
void LinearSolver :: toReal( const vector<spinX::Quaternion>& uQuat,
                             vector<double>& uReal )
...
void LinearSolver :: toQuat( const vector<double>& uReal,
                             vector<spinX::Quaternion>& uQuat )
// converts vector from real- to quaternion-valued entries
{
   for( size_t i = 0; i < uQuat.size(); i++ )
   {
      uQuat[i] = spinX::Quaternion( uReal[i*4+0],   // real
                             uReal[i*4+1],   // i
                             uReal[i*4+2],   // j
                             uReal[i*4+3] ); // k
   }
